...
- Production observation: the Cloudflare MDX screen measured 62.26 seconds with one worker, 38.63 seconds with two, 25.01 seconds with four, and 27.80 seconds with eight on a 12-logical-CPU M3 Pro. Four workers were best; eight regressed while also using more CPU and memory, but the screen does not isolate the causal mechanism. Pool readiness was about 2.0 seconds, so static hardware-derived maximum capacity is the wrong policy even for a large real transform workload. [Evidence](../experiments/cloudflare-mdx/2026-07-12-results.md#worker-count-and-sustained-execution)
