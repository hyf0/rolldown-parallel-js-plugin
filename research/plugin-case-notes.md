...
Snapshot date: 2026-07-11. These notes retain the earlier official Vite-plugin source audit and completed Vue/Svelte mechanism cases as first-iteration background. They do not select the production workload. The later production-scale iteration targeted any required JavaScript transform or transform chain that passed the roughly 5,000-distinct-module, 15–30 minute admission gate; its bounded corpus supplied no direct-Rolldown candidate, and Vue and Svelte remain controls rather than the product definition. [Production-scale verdict](./production-scale-verdict.md)
