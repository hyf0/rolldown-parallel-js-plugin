...
The scale, worker-selection, and initialization follow-up has completed its frozen protocols, harnesses, runtime attribution, final controlled and independent Vue evidence, final MDX correctness, fixed-policy terminal validation, and product-capability ledger, but it has not produced formal wall-time data. Controlled Vue, independent Vue, MDX, and generic initialization all stop before the first child because the predeclared local-host gate rejects the current uptime and swap. Consequently this snapshot makes no new claim about the Vue or MDX crossover, the best worker count, Cloudflare's worker-four to worker-eight regression, quota behavior, a default automatic policy, or repeated initialization duration. Existing numbers below remain the earlier mechanism-scale and noisy-host evidence until the restarted quiet-host matrices replace or confirm them.

The initialization work does settle two mechanism questions without wall timing. Cloudflare's approximately 1.997-second pool-ready interval is not a generic two-second worker tax because ordinary factory/configuration already took approximately 1.842 seconds in the prior single pair. Runtime `76a971de8ce66e031b7d19637d13742fe4662594` keeps generic worker creation, runtime and binding import, implementation import, plugin factory/configuration, bindingification, native registration, first/all ready, capture bounds, stage resource boundaries, termination, native close, two-pass parent garbage collection, CPU endpoints, and whole-process RSS separate. It also falsifies the hypothesis that every Node worker constructs another 18-thread Rust runtime: the `module_init` path is a dynamic-library constructor and current direct runs emit one process-level record. These fields can rank future hypotheses, but until the formal matrix runs they cannot quantify repeated wall benefit, retained-memory cost, or the best optimization.

## Recommendation

Continue bounded architecture investment, but narrow the capability around explicit worker kernels and explicit state ownership rather than treating the current whole-plugin marker as a generally compatible plugin mode. Cloudflare supplies strong repeated local evidence that four Node.js workers can more than halve a large real JavaScript transform stage, but it also shows that module count and stage speedup do not imply a 2x complete build. The plugin-managed-to-Rolldown-managed interval crosses parity, so no incremental Rolldown scheduling advantage is established. [Cloudflare result](../experiments/cloudflare-mdx/2026-07-12-results.md)
...
## Production-scale transform decision
...
The first real high-volume case now answers the stage question with a host caveat. Cloudflare's original local production path and the direct adapter both reach exactly 9,157 MDX IDs. Ten rotated local default-profile blocks reduce the direct stage from a 63.089-second ordinary median to 28.226 seconds with four Rolldown-managed workers, a paired median 2.178x with bootstrap 95% interval 2.129–2.369. A plugin-managed four-worker pool reaches 26.779 seconds, and its comparison with Rolldown crosses parity. Node.js workers have strong repeated local stage evidence; additional Rolldown-owned scheduling value is unproven. Existing swap, pageouts, worker-sample swapouts, and background load mean the matrix is not clean-host formal confirmation.

Cloudflare is a negative candidate for the requested complete-build target. Its uninstrumented local Astro reference takes 689.01 seconds, and exact transfer of the direct stage's 34.86-second saving would illustrate only about a 5% reduction. The adapter does not establish a one-to-one timing mapping, so that arithmetic is not an upper bound. Under the measured boundary, the untouched 377-second static-route phase alone exceeds half of the complete build. The overall complete-build goal remains open and depends first on transform critical-path share, then on sustained per-worker service, worker and Rust CPU allocation, ready work over time, long-task balance, retained RSS, garbage collection, memory pressure, several-plugin execution, cache determinism, and worker or task failure semantics.

The next comparison uses a Rolldown-managed shared worker group by default and an explicitly requested exclusive group for a sustained heavy plugin, both under the same global CPU and memory budget. Exclusive means a dedicated group containing one or several workers, not one worker per plugin. Colocating several plugins in shared workers remains distinct from executing their transforms as one combined worker request. Cloudflare measured one plugin-owned pool and the current Rolldown pool, but not multiple plugins, explicit shared placement, or an exclusive group. The full unsatisfied gates remain in the [production-scale goal](../.agents/docs/production-scale-goal.md).
...
| Cloudflare production MDX kernel, 9,157 transforms | 63.089 s | Rolldown worker-4, 28.226 s | 2.178x paired median | normalized output matches across ten blocks; CPU ratio 1.648 and peak-RSS ratio 1.359; plugin-managed workers reach 26.779 s; metadata and global state are lost |
...
The 2026-07-12 production-stage result does not revise historical measurements. It supplies strong repeated local evidence for a greater-than-2x stage win, rejects Cloudflare as a 2x complete-build candidate under the adapter boundary, and defines the semantic and placement work required before a public capability or minute-scale claim.
