...
- Attribution-only runtime: `76a971de8ce66e031b7d19637d13742fe4662594`; binding SHA-256 `6d6fc6e94b30b7b39b4c6d116b38bbecca2907ecc183c99a25a1a67e1cce1fce` and 16,360,800 bytes; dist SHA-256 `3e4b174ad36807430da1b5b7db3f294a47909962511531b370f421fe00d83fbd`, 49 files, and 17,240,063 bytes; `dist/index.mjs` SHA-256 `ecbce9a6cfc187db4d2c818d2500f52372b15b66022358f69c8e578c1dcbb2bc` and 1,642 bytes. Its checkout path is configurable, but `run-case.mjs` requires a clean checkout with the exact commit and all artifact descriptors.
...
- `scale-correctness-gate.json`: passed after the lifecycle-fixed full-corpus and semantic artifacts were recorded and pinned by SHA-256.
...
Both are correctness-only and emit no build wall, CPU, `/usr/bin/time`, or peak-RSS evidence. The last passed full-corpus artifact is SHA-256 `ec3281ff4c21ccb45f507a36fb8811f3d3cb195bba4de43378708f15c6b699b0`; the last passed semantic sentinel is `048db637a863b9d980620ab15c89fe1ff1e6bade2df0bfa703b3304730da1e43`. The base-screen configuration remains authorized, but the executable gate currently rejects those artifacts after the harness switched from a directory-wide manifest to an explicit correctness execution closure. One final untimed refresh is required after the remaining attribution-runtime integration; no timing child can start before it passes.
...
The attribution runner separately executes exactly one ordinary, worker-four, and worker-eight process. Every child must pass the frozen host gate and the lifecycle-fixed normalized-output oracle. Each process must emit one `createBundlerOptions` record and one native plugin-registration record with the same positive `metricsId`. Worker runs must also emit exactly one initialization/termination pair carrying that identity; every worker bootstrap and lightweight launcher must carry it too. Plugin count and index correlation is checked across the main JavaScript placeholder, native materialization, lifecycle registry, and every worker bootstrap. The Rust transform record does not yet expose `metricsId`, so it is correlated only because a fresh child permits exactly one transform record and the existing module/permit/thread bijections must all match; the report states this limit instead of implying a stronger identity.

Initialization reporting keeps `createBundlerOptions`, native registration, pool initialization, lightweight launcher imports, worker bootstrap, implementation import, plugin factory, plugin bindingification, and registry registration as separate stages. Launcher stages are nested inside the launcher-entry-through-registration bootstrap interval and must not be added as independent elapsed time. Process and main-thread CPU share exact endpoints. Worker CPU reads have asynchronous bounded endpoints, so they remain diagnostics and are never subtracted from a process interval to claim Rust/native residual CPU. The report also retains sampled and `/usr/bin/time` RSS, main and worker heap/ELU/GC, one process-level native module-init record, raw Rust arrival/acquire/complete events, permit-to-thread identity, ready and busy intervals, service distributions, widths, and throughput. Its derived per-worker service profile exposes the frozen worker-local cold call ordinals 1, 2, 4, 8, 16, and 32 plus statistics for the last 256 completed calls as the steady-service window, so initialization and JIT analysis does not have to infer cold behavior from aggregate totals. Missing, empty, mismatched, or non-derived records reject the run. Its output remains `timingEligible:false` and `conclusionEligible:false`.

A local one-source ordinary plus worker-one smoke directly executed `run-case.mjs` against the pinned attribution runtime and passed the complete `validateAttributionInitializationRecords` contract. This is only a small runtime/harness compatibility check. It bypasses formal host admission, the correctness gate and its source/runtime/harness provenance, full attribution-report validation, and the 256-call per-worker steady-service profile. It is never formal attribution evidence and cannot support timing, crossover, worker-count, startup, CPU, or RSS conclusions.
...
The old direct historical commands and unclassified `smoke-matrix.json` are intentionally rejected by the new runners. The `0aa/deec/e303` triple may be executed only by an explicitly classified, untimed `historical-replay`; retained historical artifacts remain the default source for those observations. New correctness uses the lifecycle-fixed matrices above. The correctness gate has passed, so new performance is authorized only when the frozen local-host policy also passes.
