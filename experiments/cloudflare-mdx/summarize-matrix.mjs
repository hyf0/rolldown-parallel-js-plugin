...
const provenanceRecords =
  report.runner || report.caseRunner || report.environment
    ? [report]
    : sourceReports;
const runnerHashes = provenanceRecords.map(({ runner }) => runner?.sha256);
const caseRunnerHashes = provenanceRecords.map(
  ({ caseRunner }) => caseRunner?.sha256,
);
const runnerProvenanceComplete =
  provenanceRecords.length > 0 && runnerHashes.every(Boolean);
const caseRunnerProvenanceComplete =
  provenanceRecords.length > 0 && caseRunnerHashes.every(Boolean);
const runnerProvenanceConsistent =
  runnerProvenanceComplete && new Set(runnerHashes).size === 1;
...
const environmentProvenanceComplete =
  ...
  provenanceRecords.every(({ environment }) =>
    hasCompleteEnvironmentProvenance(environment),
  );
...
const environmentPoliciesCompatible =
  environmentProvenanceComplete &&
  new Set(environmentPolicySignatures).size === 1;
const activeParentCiMarkers = provenanceRecords.flatMap(
  ({ environment }, sourceIndex) =>
    Object.entries(environment?.parentCiMarkers ?? {})
      .filter(([, value]) => isActiveCiValue(value))
      .map(([name, value]) => ({ sourceIndex, name, value })),
);
const buildProfiles = new Set(
  report.runs.map((run) => run.buildProfile ?? 'unrecorded'),
);
const effectiveRunLinkCheckRecorded = report.runs.every(
  (run) => typeof run.effectiveRunLinkCheck === 'boolean',
);
const effectiveRunLinkCheckEnabled = report.runs.some(
  (run) => run.effectiveRunLinkCheck === true,
);
const affectedHostPolicyRuns = new Set(
  runHostPolicyViolations.map(
    ({ name, index, variant }) => `${name}\0${index}\0${variant}`,
  ),
).size;
...
if (!runnerProvenanceComplete) {
  ...
  benchmarkIneligibilityReasons.push(
    'source reports used different matrix-runner revisions',
  );
}
if (!caseRunnerProvenanceComplete) {
  ...
  benchmarkIneligibilityReasons.push(
    'source reports used different case-runner revisions',
  );
}
if (!environmentProvenanceComplete) {
  ...
  benchmarkIneligibilityReasons.push(
    'source reports used different child-environment policies',
  );
}
...
function pairedStats(wallSpeedups, cpuRatios, rssRatios) {
  return {
    ...
    wallSpeedup: { ...stats(wallSpeedups), bootstrap95: bootstrapMedian(wallSpeedups) },
...
function bootstrapMedian(values) {
  const random = mulberry32(0x20260712 ^ values.length);
  const medians = [];
