...
Status: frozen before new benchmark timing or implementation changes on 2026-07-12. This protocol implements the planning rules in [scale crossover, worker selection, and initialization](./scale-crossover-worker-policy.md). [Amendment 1](./scale-crossover-protocol-amendment-1.md) supersedes the historical runtime artifact as a formal baseline after a no-timer worker-lifecycle failure. [Amendment 2](./scale-crossover-protocol-amendment-2.md) makes the MDX correctness gate executable, records the adapter's null-map capability as a product failure, and prevents screens or summaries from bypassing evidence classification. [Amendment 3](./scale-crossover-protocol-amendment-3.md) supersedes the controlled Vue schema-1 corpus with the compile-admitted 5,650-source pool and frozen 5,000-source curve. [Amendment 4](./scale-crossover-protocol-amendment-4.md) promotes the real 546-SFC Directus graph after the original independent large candidates fail admission. [Amendment 5](./scale-crossover-protocol-amendment-5.md) freezes deterministic MDX screen-to-confirmation-to-refinement selection. [Amendment 6](./scale-crossover-protocol-amendment-6.md) freezes the artifact-driven MDX Rust-pool and aggregate CPU-rate policy harness. [Amendment 7](./scale-crossover-protocol-amendment-7.md) supersedes the earlier attribution-only runtime pin, freezes correlated initialization/resource semantics, and enables full MDX attribution after the passed correctness gate. [Amendment 8](./scale-crossover-protocol-amendment-8.md) makes censored, non-monotonic, and no-crossover results executable terminal outcomes, freezes negative Vue fallback, supersedes the attribution runtime with stage-resource and post-close GC boundaries, narrows correctness provenance to its execution closure, and closes evidence-policy overclaims. A later change to a corpus, selection order, scale, acceptance threshold, host gate, runtime pin, or semantic gate requires a versioned amendment before affected timing is run. Correctness admission may reject a candidate, but it may not silently relax a gate or substitute a more favorable corpus.
