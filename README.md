...
## Production-scale candidate outcome

The production-scale `/goal` reached an inconclusive corpus result on 2026-07-12. Its finite candidate-search manifest was frozen before deep screening, with Cloudflare Docs, WordPress Gutenberg, and Elastic Kibana as the only selected candidates. All three failed the first admission rule because their production builds use Astro/Vite/Rollup, a multi-stage esbuild workflow, and webpack or Rspack respectively rather than direct Rolldown. No candidate entered the research measurement, adaptation, or implementation phases; Cloudflare's existing 17m59s CI run remains preliminary external duration evidence. The intended workload remains a required user- or ecosystem-owned JavaScript transform that cannot reasonably be replaced wholesale with Rust, roughly 5,000 distinct project modules that execute its expensive handler, a repeated 15–30 minute ordinary direct-Rolldown build, and an end-to-end target of about 30→15 or 15→7–8 minutes. The bounded search did not find that workload and therefore does not decide ParallelPlugin's production value. [Production-scale goal](./.agents/docs/production-scale-goal.md), [candidate-search manifest](./.agents/docs/production-candidate-search.md), [candidate screening](./.agents/docs/production-candidate-screening.md)

The planned execution comparison would have included ordinary main-thread execution, one-worker isolation, a plugin-managed `worker_threads` pool, a Rolldown-managed shared worker group that may host several plugins, and an explicitly exclusive group containing one or several workers. Its companion case would have compared two plugin-owned pools with one shared Rolldown group under the same total budget and measured sustained service, CPU competition, ready work, load balance, memory/JIT/cache/garbage-collection pressure, ordered transforms, coordinator semantics, adaptation cost, cache determinism, and failure behavior. No candidate passed admission, so this comparison did not open.

Candidate source screening is complete. Every selected candidate failed rule 1, so no ordinary long-running trace, plugin adaptation, parallel implementation, or plugin-managed/shared/exclusive matrix began.
...
- [Completed production-scale goal](./.agents/docs/production-scale-goal.md) records the iteration's bounded candidate search, required JavaScript workload, time and distinct-module gates, plugin-owned and Rolldown-owned worker baselines, resource and statistical protocol, adaptation cost, semantic requirements, and 2x success target.
...
- [Production candidate screening](./.agents/docs/production-candidate-screening.md) records each selected candidate's first failed admission rule, pins its evidence, and marks every later rule not evaluated.
- [Production-scale verdict](./research/production-scale-verdict.md) records the terminal inconclusive-corpus result, why no research measurement or implementation followed, what remains established, and what evidence could reopen the question.
