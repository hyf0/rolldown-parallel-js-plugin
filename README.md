...
Rolldown's experimental parallel JavaScript plugin implementation still exists, but current main is known not to work because all JavaScript callbacks use weak Node-API thread-safe functions and Node workers cannot use that lifetime mode. The retained design is still relevant: Rust already scans independent modules concurrently; ordinary JavaScript plugin callbacks are associated with the main Node.js environment, while a parallel plugin creates one plugin instance in each Node.js worker and lets Rust route concurrent module hooks to free workers.
